...
void plot_hitn_arg(const string& rootFileName){
    ...
    h_hitn = new TH1F(Form("h_hitn_%.1fGeV",beamEnergy), Form("PE hits distribution of %s with %s beam; PEs; Events/PE", geometry.c_str(), source.c_str()),
            nbins, hist_xmin, hist_xmax);
    h_hitn->SetLineColor(kBlack);
    h_hitn->SetAxisRange(0, xMaxRange);
    ...
    TString resHistText = Form("Hist: #frac{RMS}{Mean} = %.2f", resHist);
    ...
    latex.DrawLatex(0.66, 0.45, resHistText);
